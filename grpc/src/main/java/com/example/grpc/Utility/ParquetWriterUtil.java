...
public class ParquetWriterUtil {
    ...
    public static byte[] writeSpansToParquet(List<SpanData> spans) throws IOException {
        ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();

        // Use LocalOutputFile to write the data into the byte array
        try (ParquetWriter<GenericRecord> writer = AvroParquetWriter.<GenericRecord>builder(new LocalOutputFile(byteArrayOutputStream))
                .withSchema(SCHEMA)
                .withCompressionCodec(CompressionCodecName.ZSTD) // Use Zstandard compression
                .build()) {

            // Write each span to the Parquet file
            for (SpanData span : spans) {
                GenericRecord record = new GenericData.Record(SCHEMA);
                record.put("traceId", span.getTraceId());
                record.put("spanId", span.getSpanId());
                record.put("name", span.getName());
                record.put("attributes", span.getAttributes().toString());
                writer.write(record);
            }
        }

        return byteArrayOutputStream.toByteArray(); // Return the Parquet data as a byte array
    }
